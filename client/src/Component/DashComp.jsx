...
export default function DashComp() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const result = await fetch("/api/user/getusers?limit=5");
    ...
    const fetchPosts = async () => {
      try {
        const result = await fetch("/api/post/getposts?limit=5");
        const data = await result.json();
        if (result.ok) {
          setPosts(data.posts);
    ...
    const fetchComments = async () => {
      try {
        const result = await fetch("/api/comment/getComments?limit=5");
        const data = await result.json();
        if (result.ok) {
      ...
      <div className="flex-wrap flex gap-4 justify-center">
        ...
        <div className="flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md">
          <div className="flex justify-between">
            ...
            <HiDocumentText className="bg-blue-600 text-white rounded-full text-5xl p-3 shadow-lg" />
          </div>
          <div className="flex text-sm gap-2">
            <span className="text-green-500 flex items-center">
              <HiArrowNarrowUp />
        ...
        <div className="flex flex-col p-3 dark:bg-slate-800 gap-4 md:w-72 w-full rounded-md shadow-md">
          ...
          <div className="flex text-sm gap-2">
            ...
            <div className="text-gray-500">Last Comments</div>
          </div>
        </div>
      </div>
                ...
                <Table.Body key={post._id} className="divide-y">
                  <Table.Row className="bg-white dark:bg-gray-800 dark:border-gray-700">
                    <Table.Cell>
                        ...
                        src={post.image}
                        alt='post'
                        className="h-15 w-10 rounded-md bg-gray-500"
                        />
                    </Table.Cell>
